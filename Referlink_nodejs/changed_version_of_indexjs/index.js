...
app.post("/payment", async (req, res) => {
    try{
        await client.db(process.env.DATABASE).collection(process.env.COLLECTION).updateOne({_id : req.body.userid}, {$set : {isPaymentMade : true}})
        let found = await client.db(process.env.DATABASE).collection(process.env.COLLECTION).findOne({_id : req.body.userid})
